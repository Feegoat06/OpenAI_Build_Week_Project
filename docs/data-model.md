...
| `key`     | integer -7..+7                 | Key signature on the circle of fifths: sharps positive, flats negative. C=0, G=+1, F=-1, Bb=-2... | **Notation-only.** Drives enharmonic spelling (F♯ vs G♭) and which explicit accidentals VexFlow must print so the notated pitch matches the sounding pitch. It **never** mutates `chord.notes`. Transposition is a separate future feature. |
